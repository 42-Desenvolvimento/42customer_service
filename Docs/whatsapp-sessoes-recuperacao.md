...
1. `backend/src/app/index.ts` cria o servidor HTTP, executa `bootstrap(app)`,
   chama `initIO(app.server)` e somente depois inicia `StartAllWhatsAppsSessions()`.
2. `StartAllWhatsAppsSessions` busca canais ativos. Para tipo `whatsapp`, ignora
   status `DISCONNECTED` e `qrcode`; para outros canais, delega para os starters
   especificos quando as credenciais exigidas existem.
...
4. `initWbot` usa `LocalAuth` com `clientId` `wbot-<whatsapp.id>` e guarda a
   sessao no array em memoria quando recebe QR Code ou quando o client fica
   pronto.
...
Todas as rotas de sessao abaixo usam `isAuth` e estao registradas em
`backend/src/routes/whatsappSessionRoutes.ts`.
...
| `DELETE` | `/whatsappsession/:whatsappId` | Faz logout/remove a instancia em memoria e grava status `DISCONNECTED`. |

O CRUD de canais fica em `/whatsapp/` e `/whatsapp/:whatsappId`:

- `GET /whatsapp/`: lista canais do tenant.
- `GET /whatsapp/:whatsappId`: busca um canal do tenant.
- `POST /whatsapp`: cria canal e respeita `CONNECTIONS_LIMIT`.
- `PUT /whatsapp/:whatsappId`: atualiza metadados, credenciais e `chatFlowId`.
- `DELETE /whatsapp/:whatsappId`: remove a instancia em memoria, destroi o
  registro via `DeleteWhatsAppService` e emite `<tenantId>:whatsapp` com
  `action: "delete"`.

Exemplo para solicitar novo QR Code pela API autenticada:
...
| `<tenantId>:whatsapp` | `{ action: "update" | "delete", ... }` | Criacao, atualizacao ou remocao de conexoes WhatsApp. |
...
- Outros estados podem vir diretamente do WhatsApp Web por `change_state`; a
  interface deve trata-los como estado operacional da sessao.
...
### Pontos sem recuperacao automatica hoje

- `SendWhatsAppMessage` loga `SendWhatsAppMessage | Error`, mas a chamada para
  `StartWhatsAppSessionVerify` esta comentada.
- `SendMessage` envia texto ou midia a partir de registros `Message`, mas nao
  chama `StartWhatsAppSessionVerify`; erros sobem pelo job ou caller que executa
  o servico.
- `DeleteWhatsAppMessage` importa `StartWhatsAppSessionVerify`, mas a chamada
  dentro do `catch` tambem esta comentada.
- `CheckIsValidContact` referencia a verificacao em comentario, mas nao a
  executa.
